        ...
        <nav className="hidden lg:flex lg:items-center lg:space-x-6">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-sm font-semibold text-gray-700 transition-colors hover:text-[#003399]"
            >
              {link.label}
            </Link>
          ))}
            ...
            <div className="flex flex-col space-y-4 pt-8">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="text-lg font-semibold text-gray-700 transition-colors hover:text-[#003399]"
                >
                  {link.label}
                </Link>
              ))}
              <div className="pt-4">
                <Button asChild className="w-full bg-[#22C55E] font-bold hover:bg-[#16a34a]">
